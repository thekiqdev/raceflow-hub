...
export const initializeDefaultTemplates = async (): Promise<void> => {
  ...
  const defaultTemplates: CreateNotificationTemplateData[] = [
    ...
    // Password reset templates
    {
      ...
      body_html: '<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><style>body{font-family:Arial,sans-serif;line-height:1.6;color:#333;max-width:600px;margin:0 auto;padding:20px}.header{background-color:#4F46E5;color:#fff;padding:20px;text-align:center;border-radius:5px 5px 0 0}.content{background-color:#f9fafb;padding:30px;border:1px solid #e5e7eb;border-radius:0 0 5px 5px}.button{display:inline-block;padding:12px 24px;background-color:#007bff;color:#ffffff;text-decoration:none;border-radius:4px;margin:16px 0}.footer{text-align:center;padding:20px;color:#6b7280;font-size:12px}</style></head><body><div class="header"><h1>Recuperação de Senha</h1></div><div class="content"><p>Olá {{userName}},</p><p>Você solicitou a recuperação de senha da sua conta.</p><p>Clique no botão abaixo para redefinir sua senha:</p><p style="text-align:center"><a href="{{resetUrl}}" class="button">Redefinir Senha</a></p><p>Ou copie e cole o link abaixo no seu navegador:</p><p style="word-break:break-all;color:#666;background:#fff;padding:10px;border:1px solid #ddd;border-radius:4px;font-size:12px">{{resetUrl}}</p><p><strong>⚠️ Este link expira em {{expiresIn}}.</strong></p><p>Se você não solicitou esta recuperação de senha, ignore este e-mail. Sua senha permanecerá inalterada.</p></div><div class="footer"><p>Este é um e-mail automático, por favor não responda.</p><p>&copy; Cronoteam - Plataforma de gestão de corridas</p></div></body></html>',
